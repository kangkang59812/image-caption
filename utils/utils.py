...
def save_checkpoint(data_name, epoch, epochs_since_improvement, miml, decoder, decoder_optimizer,
                    bleu4, is_best):
    """
    ...
    filename = 'checkpoint_withmiml' + data_name + '.pth.tar'
...
def saveall_checkpoint(data_name, epoch, epochs_since_improvement, miml, encoder, decoder, encoder_optimizer, decoder_optimizer,
                       bleu4, is_best):
    """
    Saves model checkpoint.
    ...
    state = {'epoch': epoch,
             ...
             'decoder_optimizer': decoder_optimizer.state_dict()}
    filename = 'checkpoint_all' + data_name + '.pth.tar'
    torch.save(state, filename)
    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
        torch.save(state, 'BEST_' + filename)
...
def saveatwoLSTM_checkpoint(data_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer, decoder_optimizer,
    ...
    state = {'epoch': epoch,
             ...
             'encoder_optimizer': encoder_optimizer.state_dict(),
             'decoder_optimizer': decoder_optimizer.state_dict()}
    filename = 'checkpoint_two_lstm' + data_name + '.pth.tar'
    torch.save(state, filename)
    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
